...
public class AddMonsterDialog extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == cancelButton) {
			...
			// Get the type of monster from the drop menu and type cast to string value
			// Get the name of the attacks, hit points and attack points
			String type = (String) typeCombo.getSelectedItem();
			String attack1 = attack1Field.getText();
			String attack2 = attack2Field.getText();
			int hitPoint = (int) hitPointSpinner.getValue();
			int attack1point = (int) attack1Spinner.getValue();
			int attack2point = (int) attack2Spinner.getValue();
			Monster newMonster = null;

			// Create corresponding type of monster using Trainer.createMonster() and
			// initialise with values
			switch (type) {
			case "Water":
				try {
					newMonster = Trainer.createMonster(type, hitPoint,
							new Attack[] { new Attack(attack1, attack1point), new Attack(attack2, attack2point) });
			...
			// If there is a monster being created then add it to the trainer object and the
			// GUI menu
			if (newMonster != null) {
